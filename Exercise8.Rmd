...
![Figure 1. *Trpanosoma cruzi* in human blood](images/chaga.jpg)
...
```{r Campus Imagery, echo=TRUE, message=FALSE, warning=FALSE}
...
autoplot.OpenStreetMap(argentina) +
...
```{r leaflet simple2, fig.height=6, fig.width=8, message=FALSE, warning=FALSE, cache=FALSE}
leaflet() %>% 
  addTiles(group = "OSM")%>%
  addProviderTiles(providers$CartoDB.Positron, group = "CartoDB") %>%
  addProviderTiles(providers$Esri.NatGeoWorldMap, group = "NatGeo") %>%
